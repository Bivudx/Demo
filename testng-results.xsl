...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml">
         ...
         <!--        This is for percentage calculations     -->
                    
                    

        <!--    <xsl:variable name="failedPercent" select="format-number($failedCount div $totalCount, '###.##%')"/>  -->
       <xsl:variable name="failedPercent" select="$failedCount"/> 
   ...
   <!--         <xsl:variable name="passPercent" select="format-number($passedCount div $totalCount, '###.##%')"/>  -->
                     <xsl:variable name="passPercent" select="$passedCount"/> 
        ...
        <!--     <xsl:variable name="skipPercent" select="format-number($skippedCount div $totalCount, '###.##%')"/>  -->
                     <xsl:variable name="skipPercent" select="$skippedCount"/> 
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead"/>
                <body>
                    <h2>XSLT REPORT FOR AUTOMATION TESTING</h2>
                    <table width="100%">
                        <tr>
                    ...
                    <xsl:for-each select="$suiteElements">
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
                             <tr style="background-color: #B2FF66;"> 
                                <td width="100%">
                            ...
                            <xsl:for-each select="$testCaseElements">
                                <tr style="background-color: #C0C0C0; font-size: 12px;">
                                    <td>
                                        <xsl:value-of select="@name"/>
                                    </td>
                                    <td align="center">
    ...
    <xsl:template name="percentageOverview">
        ...
        <tr><td></td><td>Fail</td><td>Pass</td><td>Skip</td><td>Total</td><td>Pass(%)</td><td>Total Time</td></tr>
        <td></td>
        <td style="background-color: #FF0000; padding: 3px 3px 3px 0;" align="center">
          <div style="width: 50px;">
                <xsl:value-of select="$failedCount"/>
            </div>
        </td>
        <td style="background-color: #009900; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
                <xsl:value-of select="$passedCount"/>
            </div>
        </td>
        <td style="background-color:#FFFF00; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
                <xsl:value-of select="$skippedCount"/>
            </div>
        </td>
        <td align="center" style="background-color:#CCFF99; padding: 3px 3px 3px 0;">
            <div style="width: 50px;">
                <xsl:value-of select="$totalCount"/>
            </div>
        </td>
        <td align="center" style="font-weight: bold; background-color: #FF8000; padding: 3px 3px 3px 0;">
            <div style="width: 50px;">
                <xsl:value-of
        ...
        <xsl:if test="compare($testNgXslt.showRuntimeTotals, 'true') = 0">
            <td style="background-color: #CC99FF; padding: 3px 3px 3px 0;" align="center" nowrap="true">
